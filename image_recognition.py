...
def count_squares(image_path='square_image.png', canvas_region=None):
    try:
        if canvas_region:
            ...
            squares = list(pyautogui.locateAllOnScreen(image_path, confidence=0.9, region=canvas_region))
        else:
            squares = list(pyautogui.locateAllOnScreen(image_path, confidence=0.9))
