...
def draw_squares(canvas_x, canvas_y, canvas_width, canvas_height):
    ...
    for _ in range(num_squares):
        while True:
            pos = get_random_position(canvas_x, canvas_y, canvas_width, canvas_height)
    ...
    pyautogui.moveTo(canvas_x + canvas_width + 50, canvas_y + canvas_height + 50)
    return num_squares
...
def mess_up_canvas(canvas_x, canvas_y, canvas_width, canvas_height):
    ...
    for _ in range(50):
        start_x = random.randint(canvas_x + margin, canvas_x + canvas_width - margin)
        start_y = random.randint(canvas_y + margin, canvas_y + canvas_height - margin)
    ...
    canvas_after_image = pyautogui.screenshot(region=(canvas_x, canvas_y, canvas_width, canvas_height))
